...
class Sudoku:
    ...
    def fillValues(self):
        ...
        # Remove Randomly K digits to make the game
        self.removeKDigits()
    ...
    def solve(self):
        # Create a copy of the Sudoku matrix to preserve the original state
        original_mat = [row[:] for row in self.mat]

        # Attempt to solve the Sudoku puzzle
        if self.solveSudoku():
            return self.mat
        else:
            # If no solution is found, revert to the original state
            self.mat = original_mat
            return None
